...
XML-RPC can be an extremely easy way to set up a simple remote procedure call service. All you need to do is create a server instance, 
register functions with it using the register_function() method, and then launch it using the serve_forever() method. 
...
For everything else, some study is required. For instance, if you pass an instance through XML-RPC, only its instance dictionary is handled:
...
A downside to XML-RPC is its performance. The SimpleXMLRPCServer implementation is only single threaded, 
and wouldn’t be appropriate for scaling a large application, although it can be made to run multithreaded, 
as shown in “Creating a TCP Server”. Also, since XML-RPC serializes all data as XML, it’s inherently slower than other approaches. 
